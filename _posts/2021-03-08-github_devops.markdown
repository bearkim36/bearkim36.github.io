...
실제 업무에서는 더 귀찮은 과정이 있을 수 있다.  backend의 경우는 다음과 같다.

1. 개발자가 코드를 수정하고 github에 push한다.
2. 빌드한다. (요샌 typescript가 대세다)
3. 빌드된 소스를 scp를 써서 배포서버의 배포서버에 지정된 backend 경로에 복사해 넣는다.
4. ssh로 접속해서 pm2를 재시작한다.
5. 반복
...
6. 개발 조직이 소규모이다.
7. 보안상의 문제로 ssh는 pem키 파일을 써서 접속한다.
...
Backend는 typescript로 개발했고, pm2를 사용하여 운영중이다. 파일을 복사해 넣은 뒤에는 pm2를 재시작 해주는 쉘스크립트를 실행해 재시작한다.
...
$ npm run build
# 배포서버로 포팅
# scp -r build/. ssh 접속가능 계정@배포서버 IP나 DNS:pm2로 잡아놨을 backend 소스 경로
$ scp -i sshkey.pem -r build/. sshaccount@deploy.server.com:~/backend/
# 원격으로 pm2 reload
$ ssh -i sshkey.pem sshaccount@deploy.server.com:~/pm2reload.sh
```

pm2 reload 쉘스크립트는 정말 심플하다.
```sh
#pm2reload.sh
pm2 reload all
```
